...
Key Guidelines:
- You speak fluent English and Somali (Af-Soomaali). Match the language the user speaks.
- When generating posts or messages, ensure high quality:
...
- For write actions (posting, sending emails/messages), your tool call will automatically generate a confirmation preview card for the user on Telegram before anything is published.
"""
...
class AgentBrain:
    ...
    async def process_user_message(
        ...
        """
        Processes a user input message.
        Returns:
        ...
        try:
            ...
            if func_name == "post_to_facebook":
                ...
                preview = (
                    ...
                    f"{msg_content}\n\n"
                    f"🖼 *Image Attached:* {'Yes' if target_media else 'No (Text-only)'}\n\n"
                    f"_Tap below to confirm or revise._"
                )
                action_type = "post_photo" if target_media else "post_text"
                pending = confirmation_mgr.create_pending_action(
                    platform="facebook",
                    action_type=action_type,
                    payload={"message": msg_content, "caption": msg_content, "image_url": target_media},
                ...
                pending = confirmation_mgr.create_pending_action(
                    ...
                    payload={"caption": caption, "image_url": target_media},
                    preview_text=preview,
                    media_url=target_media
                )
                return preview, pending
                ...
                # Staged as Facebook first, then chained
                pending = confirmation_mgr.create_pending_action(
                    platform="facebook",
                    action_type="post_photo",
                    payload={"caption": caption, "message": caption, "image_url": target_media, "cross_post_ig": True},
                    preview_text=preview,
                    media_url=target_media
                )
                return preview, pending
